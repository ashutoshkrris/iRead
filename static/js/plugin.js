...
(function () {
      ...
      upcast: function (e, f) {
        if ("pre" == e.name) {
          for (var c = [], d = e.children, i, j = d.length - 1; 0 <= j; j--)
            (i = d[j]),
              (i.type != CKEDITOR.NODE_TEXT || !i.value.match(m)) && c.push(i);
          var g;
          if (!(1 != c.length || "code" != (g = c[0]).name))
            if (
              !(
                1 != g.children.length ||
                g.children[0].type != CKEDITOR.NODE_TEXT
              )
            ) {
  ...
  CKEDITOR.plugins.add("codesnippet", {
    ...
    lang:
      "ar,bg,ca,cs,da,de,el,en,en-gb,eo,es,et,fa,fi,fr,fr-ca,gl,he,hr,hu,it,ja,km,ko,ku,lt,lv,nb,nl,no,pl,pt,pt-br,ro,ru,sk,sl,sq,sv,th,tr,tt,ug,uk,vi,zh,zh-cn",
    icons: "codesnippet",
    hidpi: !0,
